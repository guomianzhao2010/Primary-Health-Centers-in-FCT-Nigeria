...
//Pull geojson data from the Nigeria site
...
//Write function to add 4 marker leyers: sites within sampling frame, all PHCs, general hospitals, and the teaching hospital that we are collaberating with
function markerlayer(sample, all, general, teachings){

    //Add a pop up note to indicate the name of PHCs 
    function onEachFeature(feature, layer) {
        layer.bindPopup("<h3>" + feature.properties.category +
          "</h3><hr><p>" + feature.properties.ward_name + ": "+ feature.properties.name );
      } 
    
    //Specify shape and color of the markers
    ...
    //Select PHCs within our sampling frame
    ...
    //Some sites are not included in the geojson data, so we need to add them manually
    ...
    //Merge the exisiting PHCs and missing PHCS to get a list of all sites in the sampling frame
    ...
    //Select all sites to create seperate layer
    ...
    //Select general hospitals to create seperate layer
    ...
    //Select teaching hospitals to create seperate layer
    L.geoJSON(teachings, {
        pointToLayer: function (feature, latlng) 
        {   
    ...
    // Create another layer that compares precious layers
    var compare=L.layerGroup([sample, all, general]);

    //Create a selection handle so that people can select which layer they want to look at
    var overlay={
...
//Function to extract sampled sites from the geojson data
...
//Function to extract sites not within our sampling frame from the geojson data
...
//Function to extract sites that are general hospitals from the geojson data
...
//Function to extract the teaching hospital
...
//Run the marklyer function we just wrote
...
//Write function to pick colors of the legend
...
//Write function to create the wards layer in the background
